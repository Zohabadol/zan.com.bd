...
<div class="max-w-2xl mx-auto mt-10 bg-white p-8 rounded-xl shadow-md">
    ...
    <h2 class="text-2xl font-semibold mb-6">Create Testimonial</h2>
    ...
    <form action="{{ route('admin.testimonial.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="comment" rows="3"
                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200">{{ old('comment') }}</textarea>
            @error('comment')
                <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
            @enderror
        </div>
